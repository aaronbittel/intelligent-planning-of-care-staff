...
class holtwinters:
    ...
    def __init__(self, data: pd.DataFrame,params: dict = None, smoothing_params: dict = None):
        if params is None:
        ...
        if smoothing_params is not None:
            ...
            self.smoothing_params = self.optimal_smoothing_params(self.data,self.predict_range)

    def predict(self):
        
        model_hw = ExponentialSmoothing(self.data['occupancy'],**self.params).fit(**self.smoothing_params, optimized=False)
        prediction = model_hw.forecast(self.predict_range)
        

        prediction = prediction.astype(int)
        print("Shape of test_data:", self.data['occupancy'].shape)
        ...
        for level in smoothing_levels:
            ...
            for trend in smoothing_trends:
                ...
                for seasonal in smoothing_seasonals:
                    model = ExponentialSmoothing(train_data['occupancy'],**self.params).fit(
                        smoothing_level=level, smoothing_trend=trend, smoothing_seasonal=seasonal,optimized=False)
                    ...
                    if mse < best_score:
                        best_score = mse
                        best_smoothing_params = {'smoothing_level': level, 'smoothing_trend': trend, 'smoothing_seasonal': seasonal}
...
if __name__ == "__main__":
        ...
        model = holtwinters(train_health,params)
